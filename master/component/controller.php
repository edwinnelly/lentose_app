...
";
    ...
    public function add_new_account_expenses($key_grant,$acc_name,$acc_num,$open_bal,$trans_date)
    {
        $dated = date('d-m-Y');
          $query = "INSERT INTO `money_account` (`id`, `host_key`, `acc_type`, `balance`, `created_on`, `account_number`, `cr`, `db`, `status`) VALUES (NULL, '$key_grant', '$acc_name', '$open_bal', '$trans_date', '$acc_num', '0', '0', '0')";
        $run_qry = $this->run_query($query);
        if ($run_qry == true) {
            return "success";
        } else {
    ...
    {
        $dated = date('d-m-Y');
           $query = "update money_account set acc_type='$acc_name',balance='$open_bal',created_on='$trans_date',account_number='$acc_num' where id='$pid' and host_key='$key_grant'";
          $run_qry = $this->run_query($query);
        if ($run_qry == true) {
            return "success";
        } else {
    ...
    {
        $query = "delete from money_account where id='$pid' and host_key='$key_grant' limit 1";
        return $this->runner($query);
    }
    ...
    public function edit_expenses_cate($public_key, $sid)
    {
        $query = "select * from money_account where host_key='$public_key' and id='$sid'";
        $row = $this->get_result($this->run_query($query));
        $obj = new stdClass();
        $obj->id = $row['id'];
...
public function fetch_carts123($key_grants)
    {
    ...
    {
        $query = "select * from money_account where host_key='$key_grant' and status= '0' order by id desc";
        $qx = $this->run_query($query);
        $user_list = array();
        while ($row = $this->get_result($qx)) {
            $obj = new stdClass();
